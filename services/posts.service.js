...
exports.getPostsAllComments = (data, callback) => {
    pool.query(
        `SELECT c.id_Comentario, c.Post_Id, c.Comentario , c.FechaDePublicacion, c.id_Usuario , u.Nombre, u.A_Paterno FROM Comentarios AS c INNER JOIN Usuarios AS u ON c.id_Usuario = u.id_Usuario WHERE c.Post_Id = ?`,
        [data.postId],
        (error, results, fields) => {
            if (error) {
                return callback(error);
